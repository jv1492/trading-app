#!/usr/bin/env python3
"""
Semiconductor Breakout Screener
Run: python3 semiconductor.py

Scores ~30 semiconductor stocks using the same criteria as the dashboard:
...
SEMIS = [
    "NVDA", "AMD",  "INTC", "QCOM", "MU",   "AVGO", "TXN",  "AMAT",
    "LRCX", "KLAC", "SNPS", "CDNS", "ADI",  "TSM",  "ASML", "ON",
    "MCHP", "MPWR", "SWKS", "SITM", "WOLF", "CRUS", "RMBS", "POWI",
    "UMC",  "GFS",  "ACMR", "SMTC", "SLAB", "FORM",
]
...
def main():
    print(f"\n{'='*65}")
    print(f"  Semiconductor Breakout Screener — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"  Scoring {len(SEMIS)} tickers · Same criteria as dashboard")
    print(f"{'='*65}")
    print("  Fetching data...")

    raw   = yf.download(SEMIS, period="6mo", interval="1d",
